...
<head>
    ...
    <title>Videojuegos version acortada</title>
...
<body>
    <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($tmp_consola == "") $err_consola = "La consola es obligatoria";
                ...
                $consolas_validad = ["ps4", "ps5", "switch", "xbox"];
                if(!(in_array($tmp_consola, $consolas_validad))) $err_consola = "La consola no es valida";
            ...
            if ($tmp_fecha_lanzamiento == "") $err_fecha_lanzamiento = "La fecha de lanzamiento es obligatoria";
                ...
                if(!preg_match($patron, $tmp_fecha_lanzamiento)) $err_fecha_lanzamiento = "Formato de fecha  incorrecta";
                else{
                    
                    //Forma corta de hacer la valizacion
                    $fecha_minima = "1947-01-01";
                    $fecha_maxima = date("Y-m-d", strtotime('+5 years'));
                    
                    if ($tmp_fecha_lanzamiento < $fecha_minima) $err_fecha_lanzamiento = "La fecha no puede ser menor a 1947-01-01";
                    elseif ($tmp_fecha_lanzamiento > $fecha_maxima) $err_fecha_lanzamiento = "La fecha debe ser anterior a dentro de 5 años";
    ...
    ?>
    <div class="container">
        <h1>Formulario de VideoJuegos Acortado</h1>
